...
export class ImagesService {
  ...
  public getImagesForMarker(atlasId: string, markerId: string): Observable<MarkerImage[]> {
    return this.http.get<MarkerImage[]>(this.buildApiEndpoint(`images/${atlasId}/${markerId}/markers`));
  }

  public updateImageForMarker(data: Partial<MarkerImage>): Observable<MarkerImage> {
    return this.http.post<MarkerImage>(this.buildApiEndpoint(`images/${data.atlasId}/${data.imageId}`), data);
  }
  ...
  public createPresignedURL(atlasId: string, markerId: string): Observable<{ url: string; fields: { [field: string]: string } }> {
    return this.http.get<{ url: string; fields: { [field: string]: string } }>(this.buildApiEndpoint(`images/${atlasId}/${markerId}`));
  }
  ...
  public poolForMarkerImages(atlasId: string, markerId: string, requiredCount: number): Observable<MarkerImage[]> {
    let retries = 0;
    const maxRetries = 2;
    return this.http.get<MarkerImage[]>(this.buildApiEndpoint(`images/${atlasId}/${markerId}/markers`), { context: new HttpContext().set(BYPASS_LOADER, true) }).pipe(
      expand((response: MarkerImage[]) => {
        if (response.length === requiredCount) {
          return of(response);
        }
        retries += 1;
        return timer(5000).pipe(switchMap(() => this.http.get<MarkerImage[]>(this.buildApiEndpoint(`images/${atlasId}/${markerId}/markers`), { context: new HttpContext().set(BYPASS_LOADER, true) })));
      }),
